...
int main(void) {
	...
	do
	{
		...
		printf("Base de datos de empleados ver.0\n\n"
				...
				"4)Informes\n"
				"5)SALIR\n");
		pedirEntero(&opcion, "Opcion:", "ERROR", 1, 5,2);
		switch (opcion) {
		case 1:
			...
			if(auxIndex!=-1)
			{
				...
				if(!error)
				{
					...
					error=pedirCadenaSoloLetras(auxEmploy.lastName, "Ingrese el apellido del empleado\n", "Reingrese apellido invalido\n", NOMBREAPELLIDOLENGTH,2);
					if(!error)
					{
						printf("Valores aceptados %d-%d \n", SALARIOMIN, SALARIOMAX);
						error=askFloatNumber(&auxEmploy.salary, "Ingrese el salario del Empleado", "Reingrese salario invalido\n", SALARIOMIN, SALARIOMAX,2);
						if(!error)
						{
							printf("Valores aceptados %d-%d \n",SECTORMIN, SECTORMAX);
							error=pedirEntero(&auxEmploy.sector, "Ingrese el sector del Empleado", "Reingrese Sector invalido", SECTORMIN, SECTORMAX,2);
							printf("\n\n");
						}
						if(!error)
						{
				...
				if(error)
				{
					printf("\nVolviendo al menu Intentos Agotados\n");
				}
		...
		case 2:
			...
			if(cantidadEmpleadosActivos>0)
			{
				...
				case 1:
					printf("Maximo de longitud permitido %d caracteres\n",NOMBREAPELLIDOLENGTH);
					pedirCadenaSoloLetras(auxEmploy.name, "Ingrese el nombre del empleado\n", "Reingrese nombre invalido\n", NOMBREAPELLIDOLENGTH,2);

					break;
				case 2:
					printf("Maximo de longitd permitido %d \n",NOMBREAPELLIDOLENGTH);
					pedirCadenaSoloLetras(auxEmploy.lastName, "Ingrese el apellido del empleado\n", "Reingrese apellido invalido\n", NOMBREAPELLIDOLENGTH,2);
					break;
				case 3:
					printf("Valores aceptados %d-%d \n", SALARIOMIN, SALARIOMAX);
					askFloatNumber(&auxEmploy.salary, "Ingrese el salario del Empleado", "Reingrese salario invalido\n", SALARIOMIN, SALARIOMAX,2);

					break;
				case 4:
					printf("Valores aceptados %d-%d \n",SECTORMIN, SECTORMAX);
					pedirEntero(&auxEmploy.sector, "Ingrese el sector del Empleado", "Reingrese Sector invalido", SECTORMIN, SECTORMAX,2);
					break;
				case 5:
					printf("Volviendo al menu \n");
					break;
